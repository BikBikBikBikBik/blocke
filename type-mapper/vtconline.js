...
class VtcOnlineTypeMapper {
	...
	mapTransaction(transaction) {
		return new Transaction(_.reduce(transaction.vin, (total, input) => total + input.value, 0),
			transaction.blockhash, transaction.txid, _.map(transaction.vout, (output) => ({ address: output.scriptPubKey.addresses[0], amount: output.value })),
			_.map(transaction.vin, (input) => ({ address: input.address, amount: input.value })), new Date(transaction.time * 1000));
	}
}

module.exports = new VtcOnlineTypeMapper();