...
class OptionRequestHandler {
	...
	handleUnknownRequest(unknown) {
		...
		return this.handleTransactionRequest(unknown, false, false).catch(() => this.handleBlockRequest(unknown, false, false))
		.catch(() => this.handleAccountRequest(unknown, false, false))
		.catch(() => {
			spinner.stop(true);
