...
class LiskTypeMapper {
	...
	mapNetworkInfo(network) {
		const orderedBlocks = _.sortBy(network.blocks, 'height');
		const latestBlock = _.last(orderedBlocks);
		
		return new Network(0, 0, latestBlock.height, new Date((timestampOffset + latestBlock.timestamp) * 1000));
	}
