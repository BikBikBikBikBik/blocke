...
class ZChainTypeMapper {
	...
	mapNetworkInfo(network) {
		return new Network(network.difficulty, network.hashrate, network.blockNumber);
	}
	
	mapTransaction(transaction) {
		if (transaction.vin.length === 1 && typeof(transaction.vin[0].coinbase) === 'string') {
			transaction.vin[0].retrievedVout = {
				scriptPubKey: {addresses: [resources.coinbaseAddressValue]},
		...
		return new Transaction(transaction.value, transaction.blockHash, transaction.hash, _.map(transaction.vout, (output) => ({ address: output.scriptPubKey.addresses[0], amount: output.value })),
			_.map(transaction.vin, (input) => ({ address: input.retrievedVout.scriptPubKey.addresses[0], amount: input.retrievedVout.value })), new Date(transaction.timestamp * 1000));
	}
}
