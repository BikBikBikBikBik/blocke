...
const _iquidusApiBaseAddressMap = {
	sigt: 'http://explorer.signatum.io/',
	vtc: 'https://explorer.vtconline.org/'
};
const _iquidusHashRateMultipliers = {
	sigt: 1,
	vtc: 1000000000
};
...
class IquidusClient extends ApiClientBase {
	constructor(network) {
		...
		this._hashRateMultiplier = _iquidusHashRateMultipliers[formattedNetwork];
	}
	...
	getNetworkInfo() {
		return this.executeRequest('ext/summary', 'Network Info').then((res) => {
			const network = res.data[0];
			const hashRate = parseFloat(network.hashrate);
			
			if (isNaN(hashRate)) {
				network.hashrate = '0';
			}
			
			network.hashRateMultiplier = this._hashRateMultiplier;
