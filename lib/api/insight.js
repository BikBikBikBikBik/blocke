...
const _insightApiBaseAddressMap = {
	bch: 'http://blockdozer.com/insight-api/',
...
class InsightClient extends ApiClientBase {
	...
	getBlockByNumberOrHash(blockId) {
		...
		return this.executeRequest(`block/${blockId}`, 'Block').catch(() => this.executeRequest(`block-index/${blockId}`, 'Block'))
		.then((res) => {
	...
	getNetworkInfo() {
		return this.executeRequest('status?q=getInfo', 'Network Info');
	}
