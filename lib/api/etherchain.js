...
function handleResponseErrors(res, objectName) {
	if (Array.isArray(res.data) && res.data.length === 1) {
		return res.data[0];
	} else if (!Array.isArray(res.data) && typeof(res.data) === 'object') {
		return res.data;
	}
...
class EtherchainClient extends ApiClientBase {
	...
	getNetworkInfo() {
		return this.executeRequest('basic_stats', 'Network Info').then((res) => handleResponseErrors(res, 'Network Info'));
	}
