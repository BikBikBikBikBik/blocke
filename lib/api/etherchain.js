...
class EtherchainClient extends ApiClientBase {
	...
	getAccount(accountAddress) {
		...
		return this.executeRequest(`account/${formattedAccount}`, 'Account').then((res) => handleResponseErrors(res, 'Account'));
	}
	...
	getTransaction(transactionHash) {
		return this.executeRequest(`tx/${transactionHash}`, 'Transaction').then((res) => handleResponseErrors(res, 'Transaction'));
	}
}
